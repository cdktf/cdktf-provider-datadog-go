...
func (d *jsiiProxy_Dataset) validatePutProductFiltersParameters(value interface{}) error {
	...
	case *[]*DatasetProductFilters:
		value := value.(*[]*DatasetProductFilters)
	...
	case []*DatasetProductFilters:
		value_ := value.([]*DatasetProductFilters)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktf.IResolvable, *[]*DatasetProductFilters; received %#v (a %T)", value, value)
		}
	}
